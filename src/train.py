...
train_dataset = BurstPrismaDataset(
    args.meta,
    args.npy_dir,
    train_genes,
    i_max,
...
for epoch in range(1, num_epoch):
    ...
    for batch, d in bar:
        ...
        if batch % 10 == 0:
            ...
            for target in targets:
                ...
                score, label, pred, acc, auc, ap = evaluation(out,labels)
                description += f"{target}: acc={acc:.4f}, auc={auc:.4f}, ap={ap:.4f} "
            bar.set_description(
    ...
    for target in targets:
        ...
        val_loss += criterion(val_pred, val_label.squeeze())
