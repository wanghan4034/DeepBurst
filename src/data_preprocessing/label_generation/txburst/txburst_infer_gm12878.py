...
def calculate_stats(data:pd.DataFrame):
    """
    Calculate the scRNA expression for the given data, and summarize the results into a dataframe.
    """
...
def infer_kinetics(data: pd.DataFrame, delay: float = 1, with_cell_size: bool = False):
...
def main(cell_type, data_dir, saved_dir, delay, with_cell_size=False):
    ...
    data_kinetics = infer_kinetics(data, delay=delay, with_cell_size=with_cell_size)
    ...
    merged_data.fillna(0.0).to_csv(os.path.join(saved_dir,f'{cell_type}_statistic_gene_transcript_region_delay_{delay}_with_cellsize_{int(with_cell_size)}.csv'), index = False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delay', required=False, help='Capture efficiency delay.',default=0.2,type=float)
    ...
    main(cell_type,data_dir,saved_dir,delay,with_cell_size)