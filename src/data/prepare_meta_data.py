...
meta_dataset = pd.read_csv('extra/datasets/meta_datasets/burst_bf_meta_data_E003.csv')
meta_dataset = meta_dataset.drop(['neighbors','scores'],axis=1)

data = pd.merge(meta_dataset,gene_id2neighbors,how='inner',on='gene_id')

data.to_csv('extra/datasets/meta_datasets/burst_bf_meta_data_E003.v1.csv',index=False)