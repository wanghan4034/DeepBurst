...
for eid in ["E116","E118","E003"]:
    ...
    for keep_mark in MARKS:
        ...
        for i  in tqdm(range(0,80,4)):
            ...
            for fold in [0,1,2,3]:
                ...
                if remove_marks:
                    checkpoints = f"benchmark/Promoterformer/checkpoints/{eid}.remove_{model_tag}.{fold}.No_feature_bin.mean_para.model.pt"
                else :
    ...
    predictions.to_csv(f"extra/datasets/benchmark/Promoterformer/results/{eid}_perturbation_region_predictions_mean.csv",index=False)
