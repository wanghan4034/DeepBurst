...
for eid in ["E116","E118","E003"]:
    for fold in [0,1,2,3]:
        ...
        if remove_marks:
            checkpoints = f"benchmark/Promoterformer/checkpoints/{eid}.remove_{remove_marks}.{fold}.bs_bf_para_deeptx.model.pt"
        else:
            checkpoints = f"benchmark/Promoterformer/checkpoints/{eid}.{fold}.bs_bf_para_deeptx.model.pt"
...
if remove_marks:
    predictions.to_csv(f'extra/datasets/benchmark/Promoterformer/results/remove_{remove_marks}_predictions_bs_bf_deeptx.csv',index=False)
else:
    predictions.to_csv(f'extra/datasets/benchmark/Promoterformer/results/predictions_bs_bf_deeptx.csv',index=False)
