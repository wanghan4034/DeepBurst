...
for eid in ["E116","E118","E003"]:
    ...
    for keep_mark in MARKS:
        ...
        for perturbation_strength in range(11):
            ...
            for fold in [0,1,2,3]:
                ...
                if remove_marks:
                    checkpoints = f"checkpoints/{eid}.remove_{model_tag}.{fold}.cv.model.pt"
                else :
                    checkpoints = f"checkpoints/{eid}.{fold}.cv.model.pt"
    ...
    predictions.to_csv(f"extra/datasets/results/{eid}_perturbation_predictions_cv.csv",index=False)
