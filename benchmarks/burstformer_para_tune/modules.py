...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None, bias=None):
        att_vec, att_prob = self._attention(x, mask=mask, bias=bias)
        ff_out = self.ff(att_vec)
        return self.ln(x + ff_out), att_prob
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None, bias=None):
        att_out, _ = self.self_att(x, mask, bias)
        return self.ff(att_out)
...
# class FeatureBin(nn.Module):
#     def __init__(self, bin_size, in_channels=4, out_channels=16, kernel_size=6, dilation=1, padding='same',hidden_dims = 32, *args, **kwargs) -> None:
#         super(FeatureBin,self).__init__()
#         self.conv1d_layer = nn.Conv1d(in_channels=in_channels, out_channels=hidden_dims, kernel_size=kernel_size, dilation=dilation, padding=padding)
#         self.relu1 = nn.GELU()
#         self.conv1d_layer2 = nn.Conv1d(in_channels=hidden_dims, out_channels=out_channels, kernel_size=9, dilation=1, padding='same')
#         self.relu2 = nn.GELU()
#         self.bin_layer = nn.AvgPool1d(kernel_size=bin_size, stride=bin_size)
    
#     def forward(self, x):
#         # x -> batch_size x n_marks x sequence lenth
