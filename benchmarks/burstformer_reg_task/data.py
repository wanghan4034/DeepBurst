...
class BurstFormerRegDataset(Dataset):
    ...
    def __init__(
        ...
        # labels
        self.ensg2label = (
            {r["gene_id"]: [np.log1p(r[t]) for t in targets] for r in self.meta.to_records()}
            if targets else {}
        )
    ...
    def get_sample(self, gene_id: str, add_bsz_dim: bool = False) -> Dict:
        ...
        if self.ensg2label:
            item["label"] = torch.tensor(self.ensg2label[gene_id], dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    print(dataset.get_sample('ENSG00000122417', add_bsz_dim=True))
